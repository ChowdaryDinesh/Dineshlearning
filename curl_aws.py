...
def task_1_create_a_canonical_request(
    ...
    ************* TASK 1: CREATE A CANONICAL REQUEST *************
    ...
    payload_hash = sha256_hash_for_binary_data(
        data) if data_binary else sha256_hash(data)
...
def __send_request(uri, data, headers, method, verify):
    ...
    response = requests.request(
        method, uri, headers=headers, data=data, verify=verify)
...
def load_aws_config(access_key, secret_key, security_token, credentials_path, profile):
    ...
    if access_key is None or secret_key is None:
        try:
            ...
            __log('Credentials file \'{0}\' exists \'{1}\''.format(
            ...
            __log('AWS profile \'{0}\' is missing \'{1}\''.format(
                profile, exception.args))
            raise exception
...
def inner_main(argv):
    ...
    parser.add_argument('--profile', help='AWS profile',
                        default='default', env_var='AWS_PROFILE')
